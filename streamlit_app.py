...
# ── 페이지 설정 ────────────────────────────────────────────────────
st.set_page_config(
    page_title="Finance RAG Lab · AI 에이전트",
    page_icon="🤖",
    layout="wide",
...
# Render raw HTML without Markdown's blank-line block parsing (CSS leaked as text).
st.html("""
...
# ── 사이드바 ───────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<h2 style="color:#F5C518;font-size:22px;font-weight:800;margin-bottom:4px">Finance RAG Lab</h2><p>10단원 통합 금융 학습 · AI 에이전트</p>', unsafe_allow_html=True)
    st.caption("금융 도메인에서 통합 10단원 원문을 검색합니다. 모델 미연결 시 출처를 갖춘 문서 발췌 모드로 동작합니다.")
    st.divider()
