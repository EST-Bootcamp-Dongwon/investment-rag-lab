...
from app import pattern_detection
from app.api.routes.vocabulary_exam import EXAM_OPEN_AT
from app.mongo_db import get_db

load_dotenv()
router = APIRouter()
ROOT_DIR = Path(__file__).resolve().parents[3]
FRONTEND_DIR = ROOT_DIR / "frontend" / "analysis"
GENERATED_DIR = ROOT_DIR / "app" / "generated"
QUIZ_SQL_PATH = ROOT_DIR / "app" / "quiz_seed.sql"
DOCS_DIR = ROOT_DIR / "data"
NOTEBOOK_IMAGE_DIR = FRONTEND_DIR / "images"
...
def _learn_document_map() -> dict[str, Path]:
    """Expose the bundled learning Markdown without allowing path traversal."""
    return {
        path.stem: path
...
@router.get("/api/learn/doc/{doc_id}")
...
def search_learning_documents(q: str = "", limit: int = 8) -> dict[str, object]:
    """Meilisearch를 우선 사용하고, 미구성 시 UTF-8 원문을 즉시 검색한다."""
    query = q.strip()
    if len(query) < 2:
    ...
    try:
        ...
        # t3.medium 배포에서 선택형 Meilisearch가 꺼져도 학습 검색 기능이 멈추지
        # 않도록, 번들 문서를 UTF-8로 읽는 결정적 fallback을 제공한다.
        ...
        for doc_id, path in _learn_document_map().items():
            ...
            local_hits.append({"doc_id": doc_id, "title": title, "snippet": snippet})
            if len(local_hits) >= max(1, min(limit, 20)):
                break
        return {"query": query, "hits": local_hits, "estimated_total_hits": len(local_hits), "source": "local-utf8-fallback"}
    hits = [{
...
@router.get("/api/market/sector-cloud")
...
@router.get("/api/market/chart-patterns")
...
@router.get("/api/nps/domestic-equity-holdings")
...
@router.get("/api/market/opening-session")
...
@router.get("/api/market/sox-vs-korea-semicon")
...
@router.get("/api/market/vix-vs-kospi")
...
@router.get("/api/market/circuit-breaker-history")
...
@router.get("/api/market/mdd-history")
...
@router.get("/api/market/pattern-scan")
...
@router.get("/api/market/price-volume-example")
...
@router.get("/api/market/recent-ipo-list")
...
@router.get("/api/market/ipo-since-listing")
...
@router.get("/api/market/penny-stock-rally")
...
@router.get("/api/market/dividend-history")
...
@router.get("/api/market/top-sobujang")
...
@router.post("/api/finance/company-financials")
...
@router.get("/api/home/chart-search")
